...
(defun gptel-assistant-question ()
  ...
  (interactive)
  (let* ((question (gptel-assistant-helm-read-string "Ask gptel: "))
...
(defun gptel-assistant-proof-reading-english-region-smerge ()
  ...
  (let ((prompt-prefix "Please proofread the following English text and correct any grammatical or spelling errors, please always keep the format and indentation for each line, only output the result:\n\n"))
    (gptel-assistant-rewrite-region-smerge prompt-prefix)))

(defun gptel-assistant-doc-rewrite-region-smerge ()
  "Proofread the English text in the currently selected region using gptel-assistant-call-gptel,
   and replace the selected region with the proofread text in smerge format."
  (interactive)
  (let ((prompt-prefix (gptel-assistant-helm-read-string "Input prompt for doc rewriting: ")))
    (gptel-assistant-rewrite-region-smerge (concat prompt-prefix ", for the following text, output the result only: \n\n"))))

(defun gptel-assistant-proofread-current-line-with-smerge ()
  "Proofread the current line and apply smerge formatting using gptel-assistant-proof-reading-english-region-smerge."
  (interactive)
  (save-excursion
    (beginning-of-line)
    (set-mark (point))
    (end-of-line)
    (call-interactively 'gptel-assistant-proof-reading-english-region-smerge)))
...
Otherwise, proofread the current line."
  ...
  (if (region-active-p)
      (call-interactively 'gptel-assistant-proof-reading-english-region-smerge)
    (gptel-assistant-proofread-current-line-with-smerge)))
...
(defun gptel-assistant-coding-rewrite-region-smerge ()
  "Get the prompt prefix using gptel-helm-read-string, and call gptel-assistant-rewrite-region-smerge to update the region with smerge format."
  (interactive)
  (let* ((requirement (gptel-assistant-helm-read-string "Code change requirement: "))
         (lang (gptel-assistant-get-current-language))
         ...
         (response-text (gptel-assistant-clean-markdown-response
...
(defun gptel-assistant-coding-insert-response-smerge ()
  ...
  (interactive)
  (let* ((requirement (gptel-assistant-helm-read-string "New code requirement: "))
         (lang (gptel-assistant-get-current-language))
         (prefix "You are a professional software engineer. please help me")
         (suffix (format "in %s programming language, output the code only" lang))
...
(defun gptel-assistant-coding-smart-smerge ()
  ...
  (interactive)
  (if (region-active-p)
      (call-interactively 'gptel-assistant-coding-rewrite-region-smerge)
    (call-interactively 'gptel-assistant-coding-insert-response-smerge)))
  ...
  (if (use-region-p)
      (let* ((region-start (region-beginning))
             ...
             (region-content (buffer-substring-no-properties region-start
                                                             region-end))
             (prompt (gptel-assistant-helm-read-string "Please enter your question: "))
    ...
    (let ((prompt (gptel-assistant-helm-read-string "Please enter your question: ")))
      (message (concat "Sending request to gptel... " prompt))
